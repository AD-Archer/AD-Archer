...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  let githubStats: { stars: number; forks: number; issues: number; lastPushedAt?: string } | null =
    null;
  if (githubInfo) {
    try {
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              funder: undefined,
              contributor:
                project.team?.map(m => ({ '@type': 'Person', name: m.name, url: m.link })) ||
                undefined,
          ...
          <div className="md:col-span-2 space-y-6">
                ...
                <Tabs defaultValue={defaultTab}>
                  ...
                  >
                    <TabsList className="inline-flex w-max gap-1 h-auto p-1 snap-x snap-mandatory">
                      {project.gallery && project.gallery.length > 0 && (
                        <TabsTrigger
                          value="gallery"
                          className="flex items-center gap-1 px-2 py-1 text-xs sm:text-sm whitespace-nowrap snap-start"
                        >
                          <ImagesIcon className="h-3 w-3 sm:h-4 sm:w-4" /> Gallery
                        </TabsTrigger>
                      )}
                      {project.video && (
                        <TabsTrigger
                          ...
                          className="flex items-center gap-1 px-2 py-1 text-xs sm:text-sm whitespace-nowrap snap-start"
                        >
                          <Film className="h-3 w-3 sm:h-4 sm:w-4" /> Video
                        </TabsTrigger>
                      )}
                      {project.caseStudy && (
                        <TabsTrigger
                          ...
                          className="flex items-center gap-1 px-2 py-1 text-xs sm:text-sm whitespace-nowrap snap-start"
                        >
                          <BookOpen className="h-3 w-3 sm:h-4 sm:w-4" /> Case Study
                        </TabsTrigger>
                      )}
                      {project.milestones && project.milestones.length > 0 && (
                        <TabsTrigger
                          ...
                          className="flex items-center gap-1 px-2 py-1 text-xs sm:text-sm whitespace-nowrap snap-start"
                        >
                          <ListChecks className="h-3 w-3 sm:h-4 sm:w-4" /> Milestones
                        </TabsTrigger>
                      )}
                      {project.team && project.team.length > 0 && (
                        <TabsTrigger
                          ...
                          className="flex items-center gap-1 px-2 py-1 text-xs sm:text-sm whitespace-nowrap snap-start"
                        >
                          <Users className="h-3 w-3 sm:h-4 sm:w-4" /> Team
                        </TabsTrigger>
                      )}
                      {project.architecture && (
                        <TabsTrigger
                          ...
                          className="flex items-center gap-1 px-2 py-1 text-xs sm:text-sm whitespace-nowrap snap-start"
                        >
                          <Network className="h-3 w-3 sm:h-4 sm:w-4" /> Architecture
                        </TabsTrigger>
                      )}
                      {project.github && (
                        <TabsTrigger
                      ...
                      {project.changelog && project.changelog.length > 0 && (
                        <TabsTrigger
                          ...
                          className="flex items-center gap-1 px-2 py-1 text-xs sm:text-sm whitespace-nowrap snap-start"
                        >
                          <GitBranch className="h-3 w-3 sm:h-4 sm:w-4" /> Changelog
                        </TabsTrigger>
                      )}
                    </TabsList>
                  </div>
                  ...
                  {project.gallery && project.gallery.length > 0 && (
                    <TabsContent value="gallery" id="gallery">
                      <h3 className="text-xl font-bold mb-3 flex items-center">
                        <ImagesIcon className="mr-2 h-4 w-4 text-primary" /> Gallery
                      </h3>
                  ...
                  {project.caseStudy && (
                    <TabsContent value="case">
                      <h3 className="text-xl font-bold mb-3 flex items-center">
                        <BookOpen className="mr-2 h-4 w-4 text-primary" /> Case Study
                      </h3>
                      <Tabs
                        defaultValue={
                      ...
                      >
                        <div
                          className="-mx-4 md:mx-0 overflow-x-auto sticky top-16 z-20 bg-card/80 md:bg-transparent backdrop-blur supports-[backdrop-filter]:bg-card/60 md:static md:top-auto md:z-auto border-b border-border/50 md:border-none"
                        ...
                        >
                          <TabsList className="inline-flex w-max gap-1 h-auto p-1 snap-x snap-mandatory">
                            {project.caseStudy.problem && (
                              <TabsTrigger
                                value="problem"
                                className="px-2 py-1 text-xs sm:text-sm whitespace-nowrap snap-start"
                              >
                                Problem
                              </TabsTrigger>
                            )}
                  ...
                  {project.changelog && project.changelog.length > 0 && (
                    <TabsContent value="changelog">
                      <h3 className="text-xl font-bold mb-3 flex items-center">
                        <GitBranch className="mr-2 h-4 w-4 text-primary" /> Changelog
                      </h3>
                  ...
                  {project.milestones && project.milestones.length > 0 && (
                    <TabsContent value="milestones">
                      <h3 className="text-xl font-bold mb-3 flex items-center">
                        <ListChecks className="mr-2 h-4 w-4 text-primary" /> Milestones
                      </h3>
                      <ul className="space-y-3">
                        {[...project.milestones]
                  ...
                  {project.team && project.team.length > 0 && (
                    <TabsContent value="team">
                      <h3 className="text-xl font-bold mb-3 flex items-center">
                        <Users className="mr-2 h-4 w-4 text-primary" /> Team
                      </h3>
                      <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {project.team.map((member, i) => (
                          <li
                            key={i}
                            className="flex items-center gap-3 border border-border/50 rounded-md p-3"
                          >
                            <Avatar>
                  ...
                  {project.architecture && (
                    <TabsContent value="architecture">
                      <h3 className="text-xl font-bold mb-3 flex items-center">
                        <Network className="mr-2 h-4 w-4 text-primary" /> Architecture
                      </h3>
                      ...
                      {project.architecture.images && project.architecture.images.length > 0 && (
                        <div className="mb-3">
                          <GalleryLightbox
                            images={project.architecture.images}
                  ...
                  {project.video && (
                    <TabsContent value="video">
                      <h3 className="text-xl font-bold mb-3 flex items-center">
                        <Film className="mr-2 h-4 w-4 text-primary" /> Video Walkthrough
                      </h3>
                  ...
                  {project.github && githubInfo && (
                    <TabsContent value="github">
                      <h3 className="text-xl font-bold mb-3 flex items-center">
                        <Github className="mr-2 h-4 w-4 text-primary" /> Repository
                      </h3>
                      <div className="space-y-3">
                        ...
                        {githubStats ? (
                          <div className="flex flex-wrap gap-2 mt-2">
                            ...
                            {githubStats.lastPushedAt && (
                              <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-muted border border-border/50 text-sm font-medium">
                                ...
                                <span className="ml-1">
                                  {new Date(githubStats.lastPushedAt).toLocaleDateString()}
                                </span>
                          ...
                          <p className="text-sm text-muted-foreground">
                            Stats unavailable at the moment.
                          </p>
                        )}
                      </div>
                    </TabsContent>
            ...
            <div className="bg-card/50 backdrop-blur-sm rounded-xl p-4 md:p-5 shadow-md border border-border/50">
              ...
              <div className="prose max-w-none dark:prose-invert">
                {isMarkdown(project.longDescription || project.description) ? (
                  <MarkdownContent content={project.longDescription || project.description} />
                ) : (
...
function getSimilarProjects(base: Project, others: Project[], limit = 6) {
  ...
  const scored = others.map(p => ({
    p,
    score:
      (p.tags || []).reduce((acc: number, t: string) => acc + (baseTags.has(t) ? 1 : 0), 0) +
      (p.featured ? 0.1 : 0),
  }));
  return scored
